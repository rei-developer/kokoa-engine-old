...
module.exports.topic = text => {
  ...
  text = text.replace(/(http(s)?:\/\/)?(www.)?youtube.com\/watch\?v=([a-zA-Z0-9\-_]+)(\&amp;feature=youtu.be)?(\&amp;list=[a-zA-Z0-9\-_]+)?(\&amp;index=(\d+))?(\&amp;t=(\d+))?/gi, `<p class='embed-youtube'><iframe src='//www.youtube.com/embed/$4$9' allowfullscreen='allowfullscreen'></iframe></p>`) 
  ...
  text = text.replace(/<iframe width="(\d+)" height="(\d+)" src="https:\/\/www.youtube.com\/embed\/([a-zA-Z0-9\-_]+)(\?list=[a-zA-Z0-9\-_]+)?(\?start=(\d+))?" (frameborder="(\d+)")? (allow="accelerometer;)? (autoplay;)? (encrypted-media;)? (gyroscope;)? (picture-in-picture")? (allowfullscreen(="")?)?><\/iframe>/gi, `<p class='embed-youtube'><iframe src='//www.youtube.com/embed/$3$4$5' allowfullscreen='allowfullscreen'></iframe></p>`)
