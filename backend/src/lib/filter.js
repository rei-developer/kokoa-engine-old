...
module.exports.topic = text => {
  ...
  text = text.replace(/((http(s)?:\/\/)?(www.)?instagram(\.[^(\n|\t|\s,)]+)+)+/gi, `<iframe src='https://gfycat.com/ifr/$5' frameborder='0' scrolling='no' allowfullscreen width='560' height='400'></iframe>`)
...
module.exports.post = text => {
  ...
  text = text.replace(/\[\[\s*(\d+)\]\]/gi, `<span onclick="location.href="https://히요비.asia/reader/$1" target='_blank' class='hitomiReader' style="cursor:pointer;">$1</span>`)
  return text
}